...
public class StockMarketCentralSystem implements StockMarketSubject {
	...
	private List<StockMarketObserver> observerList = new ArrayList<StockMarketObserver>();
	...
	/**
	 * Notifies all of the observers that are registered and added to the list. We
	 * notify the observers one by one sequential but this can be implemented in
	...
	public void notifyObservers() {
		...
		for (StockMarketObserver stockMarketObserver : observerList) {
			stockMarketObserver.update(stockData.getStockName(), stockData.getLastPrice(), stockData.getVolume());
		}
